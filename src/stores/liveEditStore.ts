    ...
    persist(
      immer((set, get) => ({
        ...
        addComponent: (
          ...
          const { componentOrder, _getNextOrder, _addToHistory } = get();
        ...
        reorderComponents: (newOrder: string[]) => {
          const { componentOrder, currentLayout, _addToHistory } = get();

          if (
            JSON.stringify(newOrder) === JSON.stringify(componentOrder)
          ) {
            return;
          }
          ...
          set((state) => {
            ...
            entry.changes.forEach((change) => {
              if (change.type === "update" || change.type === "move") {
                if (change.before) {
                  state.currentLayout[change.componentId] = deepClone(
                    change.before
                  );
                }
        ...
        recoverDraft: async () => {
          ...
          try {
            ...
            set((state) => {
              ...
              state.componentOrder = components.map(
                (c: ComponentState) => c.id
              );
              state.hasUnsavedChanges = true;
              state.activeDialog = null;
            });
...
export const useIsEditMode = () =>
  useLiveEditStore((state) => state.isEditMode);
export const useIsPreviewMode = () =>
  useLiveEditStore((state) => state.isPreviewMode);
...
export const useHasUnsavedChanges = () =>
  useLiveEditStore((state) => state.hasUnsavedChanges);
